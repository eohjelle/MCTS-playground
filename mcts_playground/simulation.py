...
def simulate_game(
    ...
    trajectories: Dict[PlayerType, List[TrajectoryStep]] = {
        player: [] for player in collect_trajectories_for_players
    }
    logger.debug(
        f"Simulating game with players {players} and collecting trajectories for {collect_trajectories_for_players}."
    )
    while True:
...
def randomly_assign_players(
    ...
    if len(player_list) != len(tracked_players) + len(generic_players):
        logger.warning(
            f"Number of players in state does not match number of tracked and generic players."
        )
...
def generate_trajectories(
    ...
    trajectory_player_creators: List[
        Callable[[State[ActionType, PlayerType]], TreeAgent]
    ],
    ...
    for _ in range(num_games):
        ...
        players, collect_trajectories_for_players = randomly_assign_players(
            state, trajectory_player_creators, opponent_creators, logger
        )
        new_trajectories = simulate_game(
            state, players, collect_trajectories_for_players, logger
        )
...
def benchmark(
    ...
    opponents_creators: Dict[
        str, List[Callable[[State[ActionType, PlayerType]], TreeAgent]]
    ],
    num_games: int,
    logger: logging.Logger = logging.getLogger(__name__),
) -> Dict[str, List[float]]:
    ...
    results: Dict[str, List[float]] = {
        opponent_name: [] for opponent_name in opponents_creators.keys()
    }
    for opponent_name, opponent_creator_list in opponents_creators.items():
        for _ in range(num_games):
            logger.debug(
                f"Simulating game {_ + 1} of {num_games} against {opponent_name}..."
            )
            state = initial_state_creator()
            players, main_player_set = randomly_assign_players(
                state, [main_player_creator], opponent_creator_list, logger
            )
