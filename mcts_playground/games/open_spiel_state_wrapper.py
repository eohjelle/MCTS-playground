...
class OpenSpielState(State[int, int]):
    ...
    def __init__(self, open_spiel_state, hash_board: bool = False):
        ...
        self._rewards = {
            player: 0.0 for player in self.players
        }  # Will not be correct if the state itself has rewards, but is typically correct for initial states. OpenSpiel's State.rewards() returns the cumulative rewards (since last move) for all players.
    ...
    def __hash__(self):
        if self.hash_board:
            return hash(
                str(self.spiel_state)
            )  # Hash the board instead of the serialized state, works for games where the board contains all the information.
        else:
    ...
    def rewards(self):
        # Return copy of rewards to avoid changing internal rewards attribute
