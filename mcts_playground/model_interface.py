from typing import Generic, Optional, Self, Type, Dict, Any, List
import torch
...
# TODO: Change to use generic "InputType" and "OutputType". Here, InputType should replace State[ActionType, Any] and OutputType is a direct replacement for TargetType (i. e. a renaming of the type).
# Will have to change adjacent classes for this to work, in particular TensorMapping
...
class ModelPredictor(Generic[ActionType, TargetType]):
    ...
    def __init__(
        self, model: Model, tensor_mapping: "TensorMapping[ActionType, TargetType]"
    ):
        self.model = model
    ...
    def __call__(self, state: State[ActionType, Any]) -> TargetType:
        ...
        return self.batch_call([state])[0]

    def batch_call(self, states: List[State[ActionType, Any]]) -> List[TargetType]:
        if len(states) == 0:
            return []
        device = next(self.model.model.parameters()).device
        encoded_state = self.tensor_mapping.encode_states(states, device)
        ...
        decoded_outputs = self.tensor_mapping.decode_outputs(outputs, states)
