...
class AlphaZeroValue[ActionType, PlayerType]:
    ...
    total_value: Dict[PlayerType, float] = field(
        default_factory=dict
    )  # Sum of values from all visits for each player
    visit_count: int = 0  # Number of times this node has been visited
    has_dirichlet_noise: bool = (
        False  # Whether Dirichlet noise has been added to the prior policy
    )

    def mean_value(self, player: PlayerType) -> float:
        """Mean value from all visits for a given player."""
        assert self.visit_count > 0, (
            "Cannot compute mean value for a node with no visits"
        )
...
class AlphaZeroConfig(AlgorithmParams):
    ...
    exploration_constant: float = (
        1.25  # Estimate based on values in the MuZero paper (c1=1.25, c2=19652)
    )
    ...
    discount_factor: float = (
        0.99  # Discount factor for future rewards. = 1.0 in AlphaZero paper.
    )


class AlphaZero(
    TreeSearch[ActionType, AlphaZeroValue, AlphaZeroEvaluation, PlayerType],
    ...
    def select(self, node: Node[ActionType, AlphaZeroValue, PlayerType]) -> ActionType:
        ...
        assert node.value is not None, (
            "Cannot select action from root whose value has not been set."
        )
        node_value = node.value

        if node == self.root and not node_value.has_dirichlet_noise:
            self.add_dirichlet_noise(node)

        def puct_score(
            action_child_pair: Tuple[
                ActionType, Node[ActionType, AlphaZeroValue, PlayerType]
            ],
        ) -> float:
    ...
    def evaluate(
        self, node: Node[ActionType, AlphaZeroValue, PlayerType]
    ) -> AlphaZeroEvaluation:
        """
    ...
    def update(
        self,
        node: Node[ActionType, AlphaZeroValue, PlayerType],
        action: Optional[ActionType],
        evaluation: AlphaZeroEvaluation,
    ) -> AlphaZeroEvaluation:
        """Update a node's statistics and propagate value estimates upwards."""

        policy, returns = evaluation

        # Update statistics for this node
        if node.value is None:  # Leaf node, first visit
            node.value = AlphaZeroValue(
                prior_policy=policy,
                total_value=dict(returns),
                visit_count=1,
            )
        else:
            node.value.visit_count += 1
            for player in node.state.players:
                node.value.total_value[player] += returns[player]

        # Return discounted values for the parent, using this node's immediate rewards.
        rewards = node.state.rewards()
        return policy, {
            p: rewards[p] + self._discount_factor * v for p, v in returns.items()
        }

    def full_policy(self) -> Dict[ActionType, float]:
        """Return the full policy for the root node."""
        visits = {
            action: float(child.value.visit_count if child.value else 0)
...
class AlphaZeroTrainingAdapter(TrainingAdapter[ActionType, AlphaZeroEvaluation]):
    ...
    def __init__(
        ...
        # The original AlphaZero paper doesn't use a discount factor, which is effectively the same as setting it to 1
    ...
    def extract_examples(
    ...
    ) -> List[TrainingExample[ActionType, AlphaZeroEvaluation]]:
        ...
        for i, step in enumerate(reversed(trajectory[:-1])):
            visits = {
                a: n.value.visit_count if n.value else 0
                for a, n in step.node.children.items()
            }
            ...
            value = (self.discount_factor**i) * outcome * (
                1 - self.value_softness
            ) + self.value_softness * step.node.value.mean_value(
                step.node.state.current_player
            )
