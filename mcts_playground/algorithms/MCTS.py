...
from mcts_playground.tree_search import ActionType, State, Node, TreeSearch
from mcts_playground.types import PlayerType
from .utils import (
...
class MCTS(
    TreeSearch[ActionType, MCTSValue, Dict[PlayerType, float], PlayerType],
    Generic[ActionType, PlayerType],
):
    def __init__(
        self, initial_state: State[ActionType, PlayerType], config: MCTSConfig
    ):
        self.root = Node(initial_state, value=MCTSValue(initial_state.players))
    ...
    def select(self, node: Node[ActionType, MCTSValue, PlayerType]) -> ActionType:
        ...
        def uct_score(child: Node[ActionType, MCTSValue, PlayerType]) -> float:
            if child.value is None or node.value is None:
                return float(
                    "inf"
                )  # Means unexplored children will be visited before exploitation
        ...
        return greedy_action_from_scores(
            {action: uct_score(child) for action, child in node.children.items()}
        )
        # return max(node.children.items(), key=uct_score)[0]

    def evaluate(
        self, node: Node[ActionType, MCTSValue, PlayerType]
    ) -> Dict[PlayerType, float]:
        ...
        # Average the returns
        average_returns = {
            player: value / self.num_rollouts for player, value in total_returns.items()
        }
        return average_returns

    def update(
        self,
        node: Node[ActionType, MCTSValue, PlayerType],
        action: Optional[ActionType],
        evaluation: Dict[PlayerType, float],
    ) -> Dict[PlayerType, float]:
