...
from mcts_playground.tree_search import State, Node
from mcts_playground.types import ActionType, TargetType
import torch
...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self.states = (
            None  # If not set, condition below can yield no attribute 'states' error
        )
        ...
        if write_index is not None:
            assert write_index <= self.current_size, (
                "Can not supply write index that is greater than current size."
            )
            assert write_index < self.max_size, (
                "Write index must be less than max size."
            )
    ...
    def add(
        ...
        if self.states is None:
            ...
            self.states = torch.empty(
                self.max_size, *states.shape[1:], dtype=states.dtype, device=self.device
            )
            self.targets = {
                key: torch.empty(
                    self.max_size,
            ...
            self.extra_data = {
                ...
                for key, tensor in extra_data.items()
            }

        # Verify the number of examples
        length = states.shape[0]
        for key in targets.keys():
            assert targets[key].shape[0] == length, (
                "Target tensors must have the same length as states."
            )
        for key in extra_data.keys():
            assert extra_data[key].shape[0] == length, (
                "Extra data tensors must have the same length as states."
            )
        ...
        while left_over > 0:
            ...
            for key in self.targets.keys():
                self.targets[key][start_idx:end_idx] = targets[key][
                    : end_idx - start_idx
                ]
            for key in self.extra_data.keys():
                self.extra_data[key][start_idx:end_idx] = extra_data[key][
                    : end_idx - start_idx
                ]
    ...
    def sample(
        ...
        if self.states is None or self.targets is None or self.extra_data is None:
            raise RuntimeError(
                "Cannot sample from uninitialized replay buffer. Add data first."
            )

        indices = np.random.randint(0, self.current_size, batch_size)
        return (
            self.states[indices],
            {key: self.targets[key][indices] for key in self.targets.keys()},
            {key: self.extra_data[key][indices] for key in self.extra_data.keys()},
        )
        ...
        torch.save(
            {
                ...
                "targets": {
                    key: self.targets[key][:end_idx] for key in self.targets.keys()
                }
                if end_idx > 0
                else None,
                "extra_data": {
                    key: self.extra_data[key][:end_idx]
                    for key in self.extra_data.keys()
                }
                if end_idx > 0
                else None,
    ...
    def from_file(
        ...
        # Optionally override the maximum size of the buffer.
        if max_size is not None:
            checkpoint["max_size"] = max_size
        ...
        *,
        ...
        try:
            ...
            artifact = wandb.Artifact(
                name=artifact_name, type="dataset", description=description
            )
