...
class TrainerConfig(Generic[ActionType, ModelInitParams, PlayerType, TargetType]):
    ...
    create_initial_state: Callable[
        [], State[ActionType, Any]
    ]  # Can not be a lambda function or defined in a local scope because it needs to be picklable
    checkpoint_dir: str
    ...
    # Actors
    num_actors: int = 1
    actor_opponents: List[Callable[[State[ActionType, Any]], TreeAgent]] = field(
        default_factory=list
    )  # Empty list means self-play
    actor_refresh_rate_seconds: float = (
        10.0  # Min seconds between adding examples to queue and updating model weights
    )
    actor_device: torch.device = torch.device("cpu")

    # Learner
    # Replay ratio (important parameter) is approximately learning_batch_size / learning_min_new_examples_per_step, and is usually around 1-10.
    learning_batch_size: int = 1024
    learning_min_new_examples_per_step: int = 256
    learning_min_seconds_per_step: float = 0.0  # Min seconds per step through main loop
    batches_trained_on_per_step: int = (
        1  # Number of batches to train on per step through main loop
    )
    learning_min_buffer_size: int = 10_000
    learning_device: torch.device = torch.device(
        "cuda"
    ...
    wandb_project: str | None = (
        None  # If wandb_run is None and this is not None, will create a new run in this project.
    )
    wandb_run_name: str | None = (
        None  # If wandb_run is None and wandb_project is not None, will give the run this name.
    )
    ...
    wandb_watch_log_level: Literal["all", "gradients", "parameters"] = (
        "all"  # If watch_model is True, this is the level of detail to log
    )
    wandb_watch_log_freq: int | None = (
        None  # If watch_model is True, this is the log frequency for model parameters. If None, will be set to int(learning_min_buffer_size * learning_fraction_of_buffer_examples_per_iteration / learning_batch_size)
    )
    wandb_watch_log_graph: bool = (
        True  # If watch_model is True, this is whether to log the model graph
    )
    wandb_save_artifacts: bool = (
        False  # If True, will log model, replay buffer, and checkpoints to wandb
    )
    ...
    log_file_level: Literal["DEBUG", "INFO", "WARNING", "ERROR"] = (
        "INFO"  # DEBUG will log all moves and states in individual games
    )
    log_format: str = "%(asctime)s | %(name)s | %(levelname)s: %(message)s"

    # Misc
    max_training_steps: int | None = None
    max_training_time_hours: float | None = (
        None  # If set, training will stop after this many hours.
    )
    ...
    load_model_from_path: str | None = (
        None  # If not None, __init__ will load model from this path.
    )
    load_replay_buffer_from_path: str | None = (
        None  # If not None, __init__ will load replay buffer from this path.
    )
    resume_from_wandb_checkpoint: str | None = (
        None  # If not None, __init__ will attempt to resume from wandb run with this id.
    )
    resume_from_checkpoint_path: str | None = (
        None  # If provided and not resuming from wandb_run, __init__ will attempt to load checkpoint from this path.
    )
...
def actor_worker(
    ...
    # Main loop
    logger.info(
        f"Starting main loop of actor process {actor_name} (Process ID: {os.getpid()})..."
    )
    num_self_players = max(
        len(create_initial_state().players) - len(actor_opponents), 1
    )
    last_refresh_time = time.time()
    examples = []
    total_trajectories_since_refresh = 0
    total_trajectory_steps_since_refresh = 0
    for game_count in itertools.count(1):
        try:
            ...
            trajectories = generate_trajectories(
                ...
                trajectory_player_creators=[
                    lambda state: training_adapter.create_tree_search(
                        state.clone(), model_predictor, algorithm_params
                    )
            ...
            total_trajectories_since_refresh += len(trajectories)
            total_trajectory_steps_since_refresh += sum(len(t) for t in trajectories)
            logger.debug(
                f"Collected {len(examples)} examples from game {game_count} in {time.time() - start_time:.2f} seconds."
            )
        ...
        if time.time() - last_refresh_time > actor_refresh_rate_seconds:
            try:
                ...
                examples_queue.put(
                    {
                        "data": encoded_examples,
                        "stats": {
                            "num_trajectories": total_trajectories_since_refresh,
                            "num_trajectory_steps": total_trajectory_steps_since_refresh,
                        },
                    }
                )
                logger.debug(
                    f"Added {encoded_examples[0].shape[0]} examples to queue "
                    f"from {total_trajectories_since_refresh} trajectories "
                    f"spanning {total_trajectory_steps_since_refresh} trajectory steps."
                )
            except Exception as e:
    ...
    *,
    ...
    for evaluation_count in itertools.count(1):
        ...
        try:
            ...
            eval_stats = evaluator(
                lambda state: training_adapter.create_tree_search(
                    state, model_predictor, algorithm_params
                ),
                logger=logger,
            )
            ...
            for opponent, results in eval_stats.items():
                log_message += f"\n    {opponent}: "
                log_message += ", ".join(
                    [f"{name}: {value:.4f}" for name, value in results.items()]
                )
            logger.info(log_message)
        except Exception as e:
            logger.error(f"Error in evaluation {evaluation_count}: {e}", exc_info=True)
        finally:
            time_until_next_eval = evaluator_frequency_seconds - (
                time.time() - start_time
            )
            if time_until_next_eval > 0.0:
                logger.info(
                    f"Sleeping for {time_until_next_eval:.2f} seconds until next evaluation..."
                )
...
class Trainer(Generic[ActionType, PlayerType, ModelInitParams, TargetType]):
    def __init__(
        self, config: TrainerConfig[ActionType, ModelInitParams, PlayerType, TargetType]
    ):
        self.config = config
        ...
        assert (
            config.load_model_from_path is not None or config.model_params is not None
        ), "Must specify either load_model_from_path or model_params"
        ...
        self.shared_pytorch_model = (
            config.model_architecture(**config.model_params)
            .to(config.actor_device)
            .share_memory()
        )
        ...
        self.optimizer = config.optimizer(
            self.model.model.parameters(), **config.optimizer_params
        )
        self.lr_scheduler = (
            config.lr_scheduler(self.optimizer, **config.lr_scheduler_params)
        ...
        if config.load_replay_buffer_from_path is not None:
            self.replay_buffer = ReplayBuffer.from_file(
                config.load_replay_buffer_from_path,
                device=config.learning_device,
                max_size=config.buffer_max_size,
            )
        else:
            self.replay_buffer = ReplayBuffer(
                config.buffer_max_size, device=config.learning_device
            )
        ...
        if config.wandb_run is not None:
            ...
            try:
                ...
                self.logger.info(
                    f"Initialized wandb run {self.wandb_run.project}/{self.wandb_run.name} with id {self.wandb_run.id}."
                )
            except Exception as e:
                self.logger.error(
                    f"Error initializing wandb run with settings config.wandb_project={config.wandb_project}, config.wandb_run_name={config.wandb_run_name}, config.wandb_run_id={config.wandb_run_id}: {e}",
                    exc_info=True,
                )
                self.wandb_run = None
        else:
        ...
        if self.wandb_run is not None:
            if config.wandb_use_watch:
                self.wandb_run.watch(
                    ...
                    log_freq=config.wandb_watch_log_freq or 1,
                    log_graph=config.wandb_watch_log_graph,
                )
            self.wandb_run.define_metric(
                "training_step"
            )  # Needed to log evaluation stats to correct (not latest) training step
        ...
        if config.resume_from_wandb_checkpoint is not None:
            try:
                assert self.wandb_run is not None, (
                    "Wandb run failed to initialize, so cannot resume from wandb run."
                )
                artifact = self.wandb_run.use_artifact(
                    f"{self.wandb_run.name}_checkpoint:latest"
                )
                checkpoint = torch.load(
                    f"{artifact.download()}/checkpoint.pt",
                    map_location=config.learning_device,
                )
                self.logger.info(
                    f"Loaded checkpoint from wandb run {self.wandb_run.name}."
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to load checkpoint using artifact from wandb run: {e}",
                    exc_info=True,
                )
                raise e
        if checkpoint is None and config.resume_from_checkpoint_path is not None:
            try:
                ...
                self.logger.error(
                    f"Error loading checkpoint from file: {e}", exc_info=True
                )
                raise e
        if checkpoint is None and config.resume_from_last_checkpoint:
            if os.path.exists(self.checkpoint_path):
                try:
                    checkpoint = torch.load(
                        self.checkpoint_path, map_location=config.learning_device
                    )
                    self.logger.info(f"Loaded checkpoint from {self.checkpoint_path}.")
                except Exception as e:
                    self.logger.error(
                ...
                self.logger.warning(
                    f"No checkpoint found at {self.checkpoint_path}. Training will start from scratch."
                )
    ...
    def _train_batch(self):
        ...
        states, targets, extra_data = self.replay_buffer.sample(
            self.config.learning_batch_size
        )
        ...
        with torch.autocast(device_type=self.config.learning_device.type):
            ...
            loss, metrics = self.config.training_adapter.compute_loss(
                model_outputs, targets, extra_data
            )
    ...
    def __call__(self):
        ...
        try:
            ...
            examples_queue = multiprocessing.Queue()
            actor_processes = self._run_actor_subprocesses(
                config.num_actors, examples_queue
            )
            ...
            if self.config.evaluator is not None:
                ...
                evaluator_stats_queue = multiprocessing.Queue()
                evaluator_process = self._run_evaluator_subprocess(
                    evaluator_stats_queue
                )
                processes.append(evaluator_process)
                time.sleep(
                    3.0
                )  # Some time for evaluator to start initial evaluation at step 0

            # Start main training loop
            self.logger.info(
                "Starting main training loop..."
                if self.training_step.value == 0
                else f"Resuming training from step {self.training_step.value + 1}..."
            )
            ...
            for step in itertools.count(self.training_step.value + 1):
                self.logger.info(
                    f"Training step {step}. Time elapsed: {(time.time() - start_time) / 3600:.2f} hours. Current buffer size: {len(self.replay_buffer)}."
                )
                ...
                while True:
                    while not examples_queue.empty():
                        queue_item = examples_queue.get_nowait()
                        encoded_examples, stats = (
                            queue_item["data"],
                            queue_item["stats"],
                        )
                        self.replay_buffer.add(*encoded_examples)
                        new_examples_count += encoded_examples[0].shape[0]
                        total_trajectories += stats["num_trajectories"]
                        total_trajectory_steps += stats["num_trajectory_steps"]
                    if (
                        new_examples_count < config.learning_min_new_examples_per_step
                        or len(self.replay_buffer) < config.learning_min_buffer_size
                    ):
                        self.logger.debug(
                            f"Collected {new_examples_count} examples from actors. New examples this step: {new_examples_count}. Current buffer size: {len(self.replay_buffer)}. Waiting to reach {config.learning_min_new_examples_per_step} new examples and {config.learning_min_buffer_size} buffer size. Sleeping for 1 second..."
                        )
                        time.sleep(1.0)
                    else:
                        break
                log_dict["examples_per_second"] = new_examples_count / (
                    time.time() - last_examples_time
                )
                ...
                if total_trajectories > 0:
                    log_dict["avg_trajectory_length"] = (
                        total_trajectory_steps / total_trajectories
                    )
                self.logger.info(
                    f"Collected {new_examples_count} new examples from actors. Rate of new examples: {log_dict['examples_per_second']:.2f} examples/s."
                )

                # Train on replay buffer
                temp_start_time = time.time()
                metrics_sum = {}
                for batch_count in range(1, config.batches_trained_on_per_step + 1):
                    batch_metrics = self._train_batch()
                    for name, value in batch_metrics.items():
                        metrics_sum[name] = metrics_sum.get(name, 0.0) + value
                log_dict["batches_trained_on"] = config.batches_trained_on_per_step
                log_dict["learning_time"] = time.time() - temp_start_time
                metrics = {
                    name: value / batch_count for name, value in metrics_sum.items()
                }
                log_dict.update(metrics)
                self.logger.info(
                    f"Trained on {log_dict['batches_trained_on']} batches, covering {log_dict['batches_trained_on'] * config.learning_batch_size / len(self.replay_buffer):.2%} of the buffer, in {log_dict['learning_time']:.2f} seconds. Statistics:\n    {', '.join(f'{name}: {value:.4f}' for name, value in metrics.items())}"
                )
                ...
                if (
                    self.lr_scheduler
                    and step > 0
                    and step % config.lr_scheduler_frequency == 0
                ):
                    ...
                    if new_lr != last_lr:
                        self.logger.info(
                            f"Updated learning rate with {self.lr_scheduler.__class__.__name__}. New learning rate: {new_lr:.6f}"
                        )

                # Update shared model weights
                with self.model_lock:
                    self.shared_pytorch_model.load_state_dict(
                ...
                if self.wandb_run is not None:
                    try:
                        ...
                        self.logger.error(
                            f"Error logging metrics to wandb: {e}", exc_info=True
                        )

                # Log evaluator stats to wandb
                if (
                    self.config.evaluator is not None
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Error collecting stats from evaluator: {e}", exc_info=True
                        )

                # Save checkpoint
                if (
                    config.checkpoint_frequency_hours is not None
                    and time.time() - last_checkpoint_time
                    > config.checkpoint_frequency_hours * 3600
                ) or (
                    config.checkpoint_frequency_steps is not None
                    and step % config.checkpoint_frequency_steps == 0
                ):
                    try:
                        ...
                        self.logger.error(
                            f"Error saving checkpoint: {e}", exc_info=True
                        )

                # Check exit conditions
                if (
                    config.max_training_steps is not None
                    and step >= config.max_training_steps
                ):
                    self.logger.info(
                        f"Reached maximum number of training steps ({config.max_training_steps})."
                    )
                    break
                if (
                    config.max_training_time_hours is not None
                    and time.time() - start_time > config.max_training_time_hours * 3600
                ):
                    self.logger.info(
                        f"Reached maximum training time ({config.max_training_time_hours} hours)."
                    )
                    break
                ...
                if time.time() - last_step_time < config.learning_min_seconds_per_step:
                    sleep_time = max(
                        0.0,
                        config.learning_min_seconds_per_step
                        - (time.time() - last_step_time),
                    )
                    self.logger.debug(
                        f"Have not reached minimum seconds per step ({config.learning_min_seconds_per_step} seconds). Sleeping for {sleep_time:.2f} seconds..."
                    )
            ...
            self.logger.info(
                f"Saving final checkpoint, model, and replay buffer to {self.checkpoint_path}, {self.model_path}, and {self.buffer_path}, respectively..."
            )
            self._save_checkpoint()
            ...
            if self.wandb_run is not None and self.config.wandb_save_artifacts:
                self.model.save_to_wandb(
                    wandb_run=self.wandb_run, model_name=f"{self.wandb_run.name}_model"
                )
                self.replay_buffer.save_to_wandb(
                    wandb_run=self.wandb_run,
                    artifact_name=f"{self.wandb_run.name}_buffer",
                )
                wandb.finish()
    ...
    def _save_checkpoint(self):
        ...
        checkpoint_data = {
            ...
            "replay_buffer": {
                ...
                "targets": {
                    key: self.replay_buffer.targets[key][
                        : self.replay_buffer.current_size
                    ]
                    for key in self.replay_buffer.targets.keys()
                },  # type: ignore
                "extra_data": {
                    key: self.replay_buffer.extra_data[key][
                        : self.replay_buffer.current_size
                    ]
                    for key in self.replay_buffer.extra_data.keys()
                },  # type: ignore
    ...
    def _load_checkpoint(self, checkpoint: Dict[str, Any]):
        ...
        replay_buffer_config = dict(checkpoint["replay_buffer"])
        # Allow changing the replay buffer capacity when resuming from a checkpoint by honoring the TrainerConfig.buffer_max_size value.
        if self.config.buffer_max_size is not None:
            replay_buffer_config["max_size"] = self.config.buffer_max_size
        self.replay_buffer = ReplayBuffer(
            **replay_buffer_config, device=self.config.learning_device
        )
        self.training_step.value = checkpoint["training_step"]

    def _run_actor_subprocesses(
        self, num_actors: int, examples_queue: multiprocessing.Queue
    ):
    ...
    def run_actors(self, num_actors: int, target_buffer_size: int):
        ...
        try:
            ...
            while len(self.replay_buffer) < target_buffer_size:
                ...
                time.sleep(sleep_time)
                while not examples_queue.empty():
                    encoded_examples = examples_queue.get_nowait()["data"]
                    self.replay_buffer.add(*encoded_examples)
                    new_examples_count += encoded_examples[0].shape[0]
                self.logger.info(
                    f"Collected {new_examples_count} examples from actors. Current buffer size: {len(self.replay_buffer)}."
                )
        except KeyboardInterrupt:
            self.logger.warning(f"Training interrupted by user.")
        finally:
    ...
    def run_evaluator(self):
        ...
        )  # Currently not used for anything in this method
