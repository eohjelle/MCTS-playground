...
from core.implementations.AlphaZero import AlphaZeroModelInterface, ModelOutput, AlphaZeroPrediction
...
class TicTacToeModel(AlphaZeroModelInterface[Tuple[int, int]]):
    def __init__(self):
        self.model = TicTacToeMLP()
        # Initialize weights with small random values
        for param in self.model.parameters():
            nn.init.normal_(param, mean=0.0, std=0.02)
        self.model.eval()  # Set to evaluation mode
    
    def _encode_state(self, state: TicTacToeState) -> torch.Tensor:
        """Convert board state to neural network input tensor."""
        # Create two 3x3 planes: one for X positions, one for O positions
        x_plane = torch.zeros(3, 3)
        o_plane = torch.zeros(3, 3)
    ...
    def forward(self, state: TicTacToeState) -> ModelOutput:
